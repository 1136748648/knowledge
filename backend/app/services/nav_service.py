...
from app.dal import KnowledgeNavRepository, NavContentLinkRepository, get_adapter
...
class NavService:
    def __init__(self, user: UserContext):
        adapter = get_adapter()
        self.nav_repo = KnowledgeNavRepository(adapter)
        self.link_repo = NavContentLinkRepository(adapter)
        self.user = user

    async def get_tree(self):
        nodes = await self.nav_repo.get_root_nodes()
        return [node for node in nodes if self._can_view_node(node)]

    async def get_node(self, node_id: uuid.UUID):
        return await self.nav_repo.get_by_id(node_id)

    async def create_node(self, data: NavNodeCreate):
        path = data.name
        if data.parent_id:
    ...
    async def update_node(self, node_id: uuid.UUID, data: NavNodeUpdate):
    ...
    async def link_content(self, node_id: uuid.UUID, content_type: str, content_id: str):
    ...
    def _can_view_node(self, node) -> bool:
