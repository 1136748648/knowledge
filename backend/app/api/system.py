...
@router.post("/test/storage", response_model=TestResult)
async def test_storage_connection(data: TestConnectionRequest):
    from app.services.storage_providers import StorageConfig, PROVIDER_MAP
    
    provider_name = data.configs.get("provider", "minio")
    host = data.configs.get("host", "")
    port = data.configs.get("port", "")
    
    if not host:
        return TestResult(success=False, message="Host is required")
    ...
    config = StorageConfig(
        ...
        endpoint=endpoint,
        access_key=data.configs.get("access_key", ""),
        secret_key=data.configs.get("secret_key", ""),
        bucket=data.configs.get("bucket", "wiki"),
        region=data.configs.get("region", ""),
        use_ssl=False,
    )
    ...
    try:
        ...
        display_name = provider.display_name if hasattr(provider, 'display_name') else provider_name
        
        if success:
            return TestResult(
                success=True,
                message=f"{display_name} connection successful",
            )
        else:
            return TestResult(success=False, message=f"Connection failed: {message}")
    except Exception as e:
        return TestResult(success=False, message=f"Storage test failed: {str(e)}")
...
def _write_env_file(categories: dict[str, dict[str, str]]):
    ...
    storage_cfg = categories.get("storage", {})
    if storage_cfg.get("host"):
        lines.append("# STORAGE")
        lines.append(f"STORAGE_PROVIDER={storage_cfg.get('provider', 'minio')}")
        endpoint = f"{storage_cfg.get('host')}:{storage_cfg.get('port', '9000')}" if storage_cfg.get('host') else ""
        if endpoint:
            lines.append(f"STORAGE_ENDPOINT={endpoint}")
        lines.append(f"STORAGE_ACCESS_KEY={storage_cfg.get('access_key', '')}")
        lines.append(f"STORAGE_SECRET_KEY={storage_cfg.get('secret_key', '')}")
        lines.append(f"STORAGE_BUCKET={storage_cfg.get('bucket', 'wiki')}")
        if storage_cfg.get('region'):
            lines.append(f"STORAGE_REGION={storage_cfg.get('region')}")
        lines.append(f"STORAGE_USE_SSL=false")
        lines.append("")
